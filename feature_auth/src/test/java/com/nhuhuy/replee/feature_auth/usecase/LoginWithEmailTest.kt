...
class LoginWithEmailTest {
    ...
    fun `Should return success with user id when all methods return success`() = runTest {
        ...
        } returns NetworkResult.Success(fakeAccount.id)
        ...
        //Act
        val result = useCase(fakeAccount.email, "password")
        val expected = NetworkResult.Success(fakeAccount.id)
        //Assert
        Truth.assertThat(result).isEqualTo(expected)
    }

    @Test
    fun `Should return failure when getCurrentToken returns failure`() = runTest {
        //Arrange
        coEvery {
            authRepository.loginWithEmail(fakeAccount.email, "password")
        } returns NetworkResult.Success(fakeAccount.id)
    ...
    @Test
    fun `Should return failure when updateDeviceToken returns failure`() = runTest {
        //Arrange
        coEvery {
            authRepository.loginWithEmail(fakeAccount.email, "password")
        } returns NetworkResult.Success(fakeAccount.id)
